...
# --- Criar o dataframe
df = pd.read_excel(
    io = './Arquivos do curso/Arquivos_Alunos/Datasets/system_extraction.xlsx',
    engine='openpyxl',
...
# -- Criar o sidebar
with st.sidebar:
    logo_teste = Image.open('./Arquivos do curso/Arquivos_Alunos/Mídia/logo vizion.png')
...
# Somar apenas colunas numéricas
tab1_qtde_produto = (
    tab1_qtde_produto
    .groupby('Produto vendido')[tab1_qtde_produto.select_dtypes(include='number').columns]
    .sum()
    .reset_index()
)
...
tab3_Vendas_Vendedor = (
    tab3_Vendas_Vendedor
    .groupby('Vendedor')[tab3_Vendas_Vendedor.select_dtypes(include='number').columns]
    .sum()
    .reset_index()
)

# Se quiser, ainda pode remover colunas específicas
tab3_Vendas_Vendedor = tab3_Vendas_Vendedor.drop(columns=['Nº pedido', 'Preço'], errors='ignore')
...
tab4_Venda_Cliente = (
    tab4_Venda_Cliente
    .groupby('Cliente')[tab4_Venda_Cliente.select_dtypes(include='number').columns]
    .sum()
    .reset_index()
)
